...
RECORD () {
    ...
    local cmd="$timeout parec -r --channels=1 --rate=16000 | sox -t raw -b 16 -r 16000 -e signed-integer --endian little - $1 gain $gain silence 1 $min_noise_duration_to_start $min_noise_perc_to_start 1 $min_silence_duration_to_stop $min_silence_level_to_stop pad 0.5 0.5 trim 0 5"
